...
> 🪟 **윈도우**: 새 버전이 나오면 **자동으로 업데이트**됩니다 (앱 재시작 시 적용). 따로 받을 필요 없어요.
>
> 🍎 **맥(macOS): ⚠️ 자동 업데이트가 되지 않습니다 — 반드시 수동으로 업데이트해야 합니다.**
> 새 버전이 나올 때마다 **[최신 릴리스 페이지](https://github.com/carryon-11/Habit-tracker/releases/latest)에서 새 `.dmg`를 직접 받아 다시 설치**하세요.
> _(코드 서명·공증이 없어 맥은 자동 업데이트가 비활성화돼 있습니다. Apple Developer 서명을 적용하면 맥도 자동 업데이트가 가능해집니다.)_
